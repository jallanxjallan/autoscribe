...
module.exports = async function instructionsState(params = {}) {
  ...
  function instructionRecords() {
    return app.vault.getMarkdownFiles()
      .map((file) => {
        ...
        if (!slug || declared !== "instruction") return null;
        ...
        return {
          ...
          scope: String(frontmatter.scope || "").trim(),
          component: String(frontmatter.component || "").trim(),
        };
      })
      .filter(Boolean)
  ...
  async function readInstructionRows() {
    const records = instructionRecords();
