...
fn inflight_history_preserves_source_without_moving_master() {
    ...
    fs::write(repository.path().join("One.md"), "---\nslug: cnt.one\n---\nOriginal\n").unwrap();
    ...
    assert_eq!(
        git::read_version(
            repository.path(),
            VersionRequest { revision: snapshot.commit.0, path: "One.md".into() },
        ).unwrap(),
        b"---\nslug: cnt.one\n---\nOriginal\n"
    );
}
...
fn pandoc_jobs_retain_input_order() {
    let jobs = ["first", "second"].into_iter().map(|identity| PandocJob {
...
impl TestRepository {
    ...
    fn git<const N: usize>(&self, args: [&str; N]) {
        assert!(Command::new("git").arg("-C").arg(&self.0).args(args).status().unwrap().success());
    }

    fn output<const N: usize>(&self, args: [&str; N]) -> String {
        let output = Command::new("git").arg("-C").arg(&self.0).args(args).output().unwrap();
