...
fn pending_response_uses_inflight_lineage_and_accept_commits_then_acknowledges(){
 ...
 let connection=Connection::open(&db).unwrap();let(state,writeback):(String,String)=connection.query_row("SELECT state,writeback_commit FROM response_records WHERE result_identity='call.one'",[],|row|Ok((row.get(0)?,row.get(1)?))).unwrap();assert_eq!(state,"accepted");assert!(writeback.len()>=40);fs::remove_dir_all(root).unwrap();
}
