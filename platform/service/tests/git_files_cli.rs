...
fn git_files_inspect_commit_stash_and_restore_are_owned_by_service() {
    ...
    let inspected = call(&root, json!({"version":1,"action":"inspect","paths":["one.md"]}));
    assert_eq!(inspected["items"][0]["committable"], true);
    let committed = call(&root, json!({"version":1,"action":"commit","paths":["one.md"],"message":"Editorial draft","purpose":"version"}));
    assert!(committed["commit"]["hash"].as_str().unwrap().len() >= 40);

    fs::write(root.join("one.md"), "temporary\n").unwrap();
    let stash = call(&root, json!({"version":1,"action":"stash-create","path":"one.md"}));
    let id = stash["item"]["id"].as_str().unwrap();
    fs::write(root.join("one.md"), "changed\n").unwrap();
    call(&root, json!({"version":1,"action":"stash-restore","path":"one.md","id":id}));
