...
def fanout_steps(plan: Plan) -> tuple[str, ...]:
    ...
    for fallback_number, raw_step in step_entries:
        ...
        if number in index_entries:
            raise IngestInputError(f"duplicate plan step number: {number}")
    ...
    if isinstance(raw_steps, Mapping):
        ...
        for raw_number, raw_step in raw_steps.items():
            ...
            if not isinstance(raw_step, Mapping):
                raise IngestInputError(f"plan step {number} must be an object")
...
def save_step_index(plan: Plan, entries: Mapping[int | str, str]) -> str:
    if not entries:
        raise IngestInputError("plan index must not be empty")
...
def _validate_registered_step(
    ...
    *,
    ...
    if not isinstance(engines, Mapping) or engine not in engines:
        raise IngestInputError(f"step {number} engine is not registered: {engine!r}")

    engine_record = engines[engine]
    if not isinstance(engine_record, Mapping):
        raise IngestInputError(f"step {number} engine registry record is invalid: {engine!r}")
    ...
    if engine_kind == "llm":
        ...
        if not model:
            raise IngestInputError(f"step {number} LLM model must not be empty")
        ...
        if not script:
            raise IngestInputError(f"step {number} script must not be empty")
        if not isinstance(scripts, Mapping) or script not in scripts:
            raise IngestInputError(f"step {number} script is not registered: {script!r}")

    elif engine_kind == "rag":
        profile = str(raw_step.get("rag_profile", "")).strip()
        profiles = registry.get("rag_profiles", {})
        if not profile:
            raise IngestInputError(f"step {number} RAG profile must not be empty")
        if not isinstance(profiles, Mapping) or profile not in profiles:
            raise IngestInputError(f"step {number} RAG profile is not registered: {profile!r}")

    else:
        raise IngestInputError(f"step {number} engine has unsupported kind: {engine_kind!r}")
...
def _positive_int(value: object, *, field: str) -> int:
    if isinstance(value, bool):
        raise IngestInputError(f"{field} must be an integer: {value!r}")
    try:
        number = int(value)
    except (TypeError, ValueError) as exc:
        raise IngestInputError(f"{field} must be an integer: {value!r}") from exc
    if number < 1:
        raise IngestInputError(f"{field} must be positive: {number}")
    return number
...
def _string_list(value: object, *, field: str) -> list[str]:
    ...
    if not isinstance(value, Sequence) or isinstance(value, (str, bytes, bytearray)):
        raise IngestInputError(f"{field} must be a list of strings")

    result: list[str] = []
    for item in value:
        if not isinstance(item, str) or not item.strip():
            raise IngestInputError(f"{field} must be a list of non-empty strings")
